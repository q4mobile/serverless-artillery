...
`,
  ...
  validateServiceForDeployment: (service) => {
    if (
      ...
      const isMonitoringEvent = event => (event && event.schedule && event.schedule.name && event.schedule.name === constants.ScheduleName)
  ...
  validateServiceForInvocation: (options, script, cwd = process.cwd) => {
    ...
    try {
      ...
      toss = !('version' in localService) && (options.monitoring || modes.isMonitoringScript(script))
    } catch (ex) {
  ...
  populateArtilleryLambda: (tempPath) => {
    ...
    constants.ServerlessFiles.forEach((file) => {
      try {
        ...
        console.error(`Failed to copy ${file} from ${sourceArtilleryLambdaPath} to ${tempPath}`)
      }
    })

    if (anyFailedCopies) throw new Error(`Failed to populate artillery project in ${tempPath}.`)
  },
   ...
   */
  writeUserSAConfig: config => fs.writeFileSync(constants.UserConfigPath, JSON.stringify(config)),
  ...
  tempArtilleryLambda: () => {
    ...
    if (impl.verifyTempArtilleryLambda(userConfig.tempPath)) {
      ...
      if (process.env.DEBUG) console.log(`Using existing default Artillery Lambda assets in ${userConfig.tempPath} ...`)
    ...
    try {
      if (process.env.DEBUG) console.log(`Creating new default Artillery Lambda assets in ${tempPath} ...`)
        ...
        return lambda.putFunctionConcurrency(params).promise()
          .catch((err) => {
            ...
            if (err.code === 'ResourceNotFoundException') {
              throw new Error(`${os.EOL}Kill command failed: The function ${funcName} is not deployed${os.EOL}`)
              ...
              throw new Error(`${os.EOL}Unexpected error setting concurrency to zero: ${err.message} ${err.code}${os.EOL}`)
            }
          })
      })
      .then(() => {
        ...
        return module.exports.remove(options)
          .then(() => {
            ...
            console.log(`${os.EOL}We advise to wait until ${deployTime.toLocaleTimeString('en-US')} before re-deploying to avoid possible problems. See https://github.com/Nordstrom/serverless-artillery#killing-in-progress-performance-test${os.EOL}`)
          })
      })
  },
