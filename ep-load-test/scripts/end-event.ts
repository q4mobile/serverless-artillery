...
import { buildUpdateEventStatusBody } from './event-status-graphql';
...
interface EventStatusResponse {
  data?: {
    updateEventStatus?: { meetingId?: number; title?: string; status?: string } | null;
  };
...
async function callEndEvent(
  ...
  const body = JSON.stringify(buildUpdateEventStatusBody(meetingId, 'ENDED'));
...
export async function endEvents(
  ...
  for (const [i, meetingId] of config.meetingIds.entries()) {
    writeJsonLog(deps, {
      lvl: 'INFO',
      evt: 'ep.event.end.begin',
      msg: 'Ending event',
      meetingId,
      index: i,
    });

    const res = await callEndEvent(deps, url, headers, meetingId, config.fetchTimeoutMs);
    ...
    if (!res.ok) {
      ...
      writeJsonLog(deps, {
        lvl: 'ERROR',
        evt: isAuth ? 'ep.event.end.unauthorized' : 'ep.event.end.failed',
        msg: isAuth ? 'Auth rejected; refresh Q4_ADMIN_TOKEN' : 'updateEventStatus(ENDED) failed',
        meetingId,
        status: res.status,
      ...
      try {
        const body = parseLabeledJson<EventStatusResponse>(res.text, 'updateEventStatus response');
        if (body.errors?.length) throw new Error(body.errors[0]?.message ?? 'GraphQL returned errors');
        const result = body.data?.updateEventStatus;
        succeeded.push(meetingId);
        writeJsonLog(deps, {
          lvl: 'INFO',
          evt: 'ep.event.end.success',
          msg: 'Event ended',
          meetingId,
          title: result?.title,
        ...
        writeJsonLog(deps, {
          lvl: 'ERROR',
          evt: 'ep.event.end.invalid_response',
  ...
  writeJsonLog(deps, {
    ...
    evt: 'ep.event.end.complete',
    msg: 'Event end complete',
...
async function runCli(): Promise<void> {
  const result = await endEvents(loadStartConfigFromEnv());
...
function isCliEntry(): boolean {
  ...
  try {
    ...
    return /end-event\.[tj]s$/i.test(String(argv1).replace(/\\/g, '/'));
  }
}
